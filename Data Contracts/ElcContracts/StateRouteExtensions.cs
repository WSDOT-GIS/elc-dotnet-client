...
namespace Wsdot.Elc.Contracts
{
	...
	public static class StateRouteExtensions
	{
		...
		internal static RrtDict RrtDescriptions
		{
			get
			{
				if (_rrtDescriptions == null)
				{
					...
					using (var reader = new StringReader(Resources.RrtList))
					{
		...
		/// If <paramref name="rrt"/> is one of the RRTs defined in <see cref="Resources.RrtList"/>
		...
		{
			if (string.IsNullOrEmpty(rrt) || string.Compare(rrt, "ML", true) == 0)
			{
				return "Mainline";
			}
			else
